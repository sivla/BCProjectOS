...
$gates=@('initialization','planning','setup','migration','process_execution','test_uat','customer_domain_approval','uat_signoff','cutover_go','go_live_rehearsal','hypercare_acceptance','restart','handover');$gates|% {[ordered]@{gate=$_;role='synthetic-reviewer';criteria="synthetic-$($_)-criteria-met";decision='GO';evidence_id="EVD-SIM-$($_)";status='simulated_pass';synthetic=$true;external_activity=$false}}|ConvertTo-Json -Depth 8|Set-Content (Join-Path $dest 'evidence\simulated-gates.json') -Encoding utf8
