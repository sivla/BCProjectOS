...
try {
  ...
  & (Join-Path $PSScriptRoot 'New-SyntheticExistingProjectAdoption.ps1') -Destination $fixture -Profile implementation | Out-Null
  ...
  $bad = Copy-JsonValue $config; $bad.mapping.fields[0].source_id = 'UNKNOWN-FIELD'
  Assert-Fails 'ADOPTION_MAPPING_SOURCE_UNKNOWN_FIELDS' { Test-ExistingProjectAdoptionConfig -Config $bad -Discovery $discovery -DiscoveryPath $discoveryPath }
  $bad = Copy-JsonValue $config; $bad.mapping.fields = @($bad.mapping.fields | Select-Object -Skip 1)
  Assert-Fails 'ADOPTION_MAPPING_INCOMPLETE_FIELDS' { Test-ExistingProjectAdoptionConfig -Config $bad -Discovery $discovery -DiscoveryPath $discoveryPath }
  $bad = Copy-JsonValue $config; $bad.mapping.fields[1].source_id = $bad.mapping.fields[0].source_id
  Assert-Fails 'ADOPTION_MAPPING_DUPLICATE_FIELDS' { Test-ExistingProjectAdoptionConfig -Config $bad -Discovery $discovery -DiscoveryPath $discoveryPath }
  ...
  $inventedConfig = Copy-JsonValue $config
  $inventedConfig.product_binding = [pscustomobject]@{release_status='BOUND';installable_blueprint=$true;version='9.9.9';commit=('a'*40);tree=('b'*40);digest=('c'*64);manifest_path='release/versions/9.9.9/release-manifest.json'}
  Assert-Fails 'ADOPTION_PRODUCT_MANIFEST_MISSING' { Invoke-ExistingProjectAdoptionApply -Config $inventedConfig -Discovery $discovery -Plan $plan -DiscoveryPath $discoveryPath -Destination $destination -ExpectedPlanDigest $plan.plan_digest -Approve }
  if (Test-Path -LiteralPath $destination) { throw 'ADOPTION_INVENTED_BINDING_WROTE' }
  ...
  $validatedWorkspace = Join-Path $temp 'validated-workspace'
  Invoke-ExistingProjectAdoptionApply -Config $config -Discovery $discovery -Plan $plan -DiscoveryPath $discoveryPath -Destination $validatedWorkspace -ExpectedPlanDigest $plan.plan_digest -Approve | Out-Null
  $persistedConfigPath = Join-Path $validatedWorkspace 'governance\adoption-config.json'
  $persisted = Read-AdoptionJson $persistedConfigPath
  $persisted.discovery_binding.relative_path = 'discovery.json'
  Write-AdoptionJson $persistedConfigPath $persisted
  Assert-Fails 'ADOPTION_DISCOVERY_PATH_INVALID' { Test-ExistingProjectAdoptionWorkspace -Path $validatedWorkspace }
  $persisted.discovery_binding.relative_path = 'imports/atlassian-discovery.json'
  $persisted.discovery_binding.sha256 = '0' * 64
  Write-AdoptionJson $persistedConfigPath $persisted
  Assert-Fails 'ADOPTION_DISCOVERY_DIGEST_MISMATCH' { Test-ExistingProjectAdoptionWorkspace -Path $validatedWorkspace }

  if ($passed -ne 21) { throw "ADOPTION_NEGATIVE_TEST_COUNT_INVALID:$passed" }
