...
function Remove-Prop($o,$n){$o.PSObject.Properties.Remove($n)}
function Unknown-Ref($x,$domain){$x.timeline[0].references=@("UNKNOWN-$domain")}
function Drop-Edge($x){$x.graph=@($x.graph|Select-Object -First 1)}
$cases=@(
 @{code='PORTABLE_PAGE_METADATA_PARENT';m={param($x)};fileMut={param($d)$p=Join-Path $d 'pages/PAGE-PORT-02.md';(Get-Content $p -Raw)-replace 'parent:\s*PAGE-PORT-01','parent: UNKNOWN'|Set-Content $p -Encoding utf8}},
 ...
 @{code='PORTABLE_DUPLICATE_SOURCEPATH';m={param($x)$x.pages[1].parent=$null;$x.pages[1].sourcePath=$x.pages[0].sourcePath}},
 ...
 @{code='PORTABLE_SCHEMA_MINITEMS';m={param($x)$x.tickets[0].comments=@($x.tickets[0].comments|Where-Object type -ne closing)}},
 ...
 @{code='PORTABLE_TIMELINE_REFERENCE';m={param($x)Unknown-Ref $x page}},
 @{code='PORTABLE_TIMELINE_REFERENCE';m={param($x)Unknown-Ref $x ticket}},
 @{code='PORTABLE_TIMELINE_REFERENCE';m={param($x)Unknown-Ref $x evidence}},
 @{code='PORTABLE_TIMELINE_REFERENCE';m={param($x)Unknown-Ref $x session}},
 @{code='PORTABLE_TIMELINE_REFERENCE';m={param($x)Unknown-Ref $x decision}},
 @{code='PORTABLE_TIMELINE_REFERENCE';m={param($x)Unknown-Ref $x deliverable}},
 ...
 @{code='PORTABLE_GRAPH_DUPLICATE';m={param($x)$x.graph=@($x.graph+$x.graph[0])}},
 @{code='PORTABLE_GRAPH_INVERSE';m={param($x)Drop-Edge $x}},
 @{code='PORTABLE_GRAPH_RELATION';m={param($x)$x.graph[1].type='unknown'}},
 @{code='PORTABLE_UNSAFE_PATH';m={param($x)$x.pages[0].sourcePath='../secret.txt'}}
)
foreach($c in $cases){
 $d=Join-Path ([IO.Path]::GetTempPath()) ('portable-neg-'+[guid]::NewGuid().ToString('N'))
 try{
  & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'automation/New-PortableProjectStoryFixture.ps1') -Destination $d|Out-Null
  $x=Get-Content (Join-Path $d 'project-story.json') -Raw|ConvertFrom-Json
  & $c.m $x
  $x|ConvertTo-Json -Depth 40|Set-Content (Join-Path $d 'project-story.json')
  if($c.fileMut){& $c.fileMut $d}
  $r=Invoke-V $d
  if($r.Exit -ne 1 -or $r.Err -ne '' -or $r.Raw -cne $c.code){throw "PORTABLE_NEGATIVE_EXPECTED_$($c.code):$($r.Raw)"}
  Write-Host "PASS: $($c.code)"
 }finally{if(Test-Path $d){Remove-Item $d -Recurse -Force -ErrorAction SilentlyContinue}}
}
$self=Join-Path ([IO.Path]::GetTempPath()) ('portable-self-'+[guid]::NewGuid().ToString('N'))
try{& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root 'automation/New-PortableProjectStoryFixture.ps1') -Destination $self|Out-Null;$sx=Get-Content (Join-Path $self 'project-story.json') -Raw|ConvertFrom-Json;$sx.offer.actual_cost=1;$sx|ConvertTo-Json -Depth 40|Set-Content (Join-Path $self 'project-story.json');$sr=Invoke-V $self;if($sr.Exit -ne 1 -or $sr.Raw -cne 'PORTABLE_COST_MISMATCH'){throw 'PORTABLE_HARNESS_ORACLE_SELFTEST_FAILED'};Write-Host 'PASS: PORTABLE_HARNESS_ORACLE_SELFTEST'}finally{if(Test-Path $self){Remove-Item $self -Recurse -Force -ErrorAction SilentlyContinue}}
