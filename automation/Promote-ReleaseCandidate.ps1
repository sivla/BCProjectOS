...
if([int]$manifest.schema_version-ne2-or[string]$manifest.product_id-ne'spectra'-or[string]$manifest.release_version-ne$Version-or[string]$manifest.manifest_state-ne'candidate'-or[string]$manifest.release_kind-ne'installable_blueprint'-or[string]$manifest.consumer_mode-ne'CONTRACT_REFERENCE_ONLY'-or$manifest.installable_blueprint-ne$false-or[string]$manifest.source_commit-notmatch'^[0-9a-f]{40}$'-or[string]$manifest.source_tree-notmatch'^[0-9a-f]{40}$'){throw 'CANDIDATE_NOT_BOUND_OR_INCONSISTENT'}
$candidateSource=[string]$manifest.source_commit;$treeOutput=@(& git -C $root rev-parse "$candidateSource`^{tree}" 2>$null);$treeExit=$LASTEXITCODE;$actualTree=([string]($treeOutput|Select-Object -First 1)).Trim();if($treeExit-ne0-or$actualTree-ne[string]$manifest.source_tree){throw 'CANDIDATE_SOURCE_TREE_MISMATCH'}
...
$scope=Get-BCProjectOSReleaseScope -Root $root;$sourceRecords=@(Get-BCProjectOSGitPayloadRecords -Root $root -Revision $candidateSource -Scope $scope);$headRecords=@(Get-BCProjectOSGitPayloadRecords -Root $root -Revision $head -Scope $scope);$expected=Get-BCProjectOSChecksumsText $sourceRecords
if((Get-BCProjectOSChecksumsText $headRecords)-ne$expected){throw 'PAYLOAD_CHANGED_AFTER_SOURCE_COMMIT'};$actual=([IO.File]::ReadAllText($checksumsPath)-replace"`r`n","`n");if($actual-ne$expected){throw 'CANDIDATE_CHECKSUM_MISMATCH'};$digest=Get-BCProjectOSTextSha256 $expected;if([string]$manifest.payload.bundle_digest-ne$digest){throw 'CANDIDATE_DIGEST_MISMATCH'}
$finalSource=$head;$finalTree=(& git -C $root rev-parse "$finalSource`^{tree}").Trim();$final=[ordered]@{};foreach($p in $manifest.PSObject.Properties){$final[$p.Name]=$p.Value};$final.manifest_state='final';$final.source_commit=$finalSource;$final.source_tree=$finalTree;$final.consumer_mode='INSTALLABLE_BLUEPRINT';$final.installable_blueprint=$true;$final.expected_tag=$tagName
