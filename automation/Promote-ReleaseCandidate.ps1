...
if(-not $SyntheticRepository){throw 'SYNTHETIC_REPOSITORY_REQUIRED: Promotion is disabled for real repositories.'}
...
$tagCommit=(& git -C $root rev-parse "refs/tags/$tagName^{commit}").Trim();if($tagCommit-ne (& git -C $root rev-parse HEAD).Trim()){throw 'TAG_COMMIT_MISMATCH'};Write-Host "PASS: Promoted Spectra $Version in synthetic repository.";Write-Host "SOURCE_COMMIT=$sourceCommit";Write-Host "TAG_COMMIT=$tagCommit";Write-Host "PAYLOAD_DIGEST=$digest"
