...
try{
  ...
  $applied=& (Join-Path $PSScriptRoot 'Invoke-Spectra.ps1') -Command init -Workspace $workspace -Guided -AnswersPath $standardPath -Apply|ConvertFrom-Json
  ...
  $customWorkspace=Join-Path $tmp 'custom-workspace';& (Join-Path $PSScriptRoot 'Invoke-Spectra.ps1') -Command init -Workspace $customWorkspace -Guided -AnswersPath $customPath -Apply|Out-Null
