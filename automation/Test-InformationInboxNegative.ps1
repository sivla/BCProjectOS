...
function New-Case([string]$name,[scriptblock]$mutation,[string]$code) {
  ...
  try {
    ...
    $jsonPath = Join-Path $d 'information-inbox.json'
    $x = [IO.File]::ReadAllText($jsonPath, [Text.Encoding]::UTF8) | ConvertFrom-Json
    & $mutation $x $d
    [IO.File]::WriteAllText($jsonPath, (($x | ConvertTo-Json -Depth 40) + "`n"), $utf8)
    $r=Invoke-Exact $d
