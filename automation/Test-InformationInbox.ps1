...
$value = Get-Content (Join-Path $d 'information-inbox.json') -Raw | ConvertFrom-Json
...
foreach ($snapshot in (Get-Items $value.snapshots)) { if ($snapshot.read_only -ne $true -or -not $sourceKeys.ContainsKey("meeting|SRC-SYN-001|$($snapshot.source_revision)")) { Fail 'INBOX_PROVENANCE_REQUIRED' } }
