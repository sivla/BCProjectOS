...
# Klassifiziere bekannte fachliche Vertragsverletzungen vor der generischen
# Schemaebene, damit die dokumentierten stabilen Codes nicht von einem
# allgemeineren Schemafehler verdeckt werden.
foreach($candidateBlueprint in @($catalog.blueprints)) {
  if($candidateBlueprint.kind -notin @('implementation','support-only','bc-basic')) { Fail 'BLUEPRINT_KIND_UNKNOWN' }
  if($null -ne $candidateBlueprint.provenance -and [string]::IsNullOrWhiteSpace([string]$candidateBlueprint.provenance.revision)) { Fail 'BLUEPRINT_PROVENANCE_REQUIRED' }
}
foreach($candidate in @($catalog.blueprint_candidates)) {
  if($candidate.review_status -ne 'pending') { Fail 'BLUEPRINT_CANDIDATE_STATUS_INVALID' }
}
foreach($candidateTree in @($catalog.page_trees)) {
  foreach($candidateNode in @($candidateTree.nodes)) {
    if([string]::IsNullOrWhiteSpace([string]$candidateNode.title)) { Fail 'BLUEPRINT_PAGE_REQUIRED_EMPTY' }
  }
}
...
$seen=@{}
$legacyCatalogPath=Join-Path $PSScriptRoot '..\contract\blueprints\catalog.json'
if(-not(Test-Path -LiteralPath $legacyCatalogPath -PathType Leaf)){Fail 'BLUEPRINT_LEGACY_CATALOG_MISSING'}
$legacyIds=@((Get-Content -LiteralPath $legacyCatalogPath -Raw|ConvertFrom-Json).blueprints|ForEach-Object{[string]$_.id})
foreach($bp in @($catalog.blueprints)){
  ...
  if(@($bp.legacy_package_refs).Count -eq 0 -or @($bp.legacy_package_refs|Group-Object|Where-Object Count -gt 1).Count -gt 0){Fail 'BLUEPRINT_LEGACY_MAPPING_AMBIGUOUS'}
  foreach($legacyRef in @($bp.legacy_package_refs)){if($legacyIds -notcontains [string]$legacyRef){Fail 'BLUEPRINT_LEGACY_REF_UNKNOWN'}}
