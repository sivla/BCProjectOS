...
$audit = @(
  ...
  [ordered]@{id='AUD-REVIEW-001';event_type='proposal_review';actor_role='reviewer';occurred_at='2026-07-13T09:20:00Z';source_revision='REV-001';before=[ordered]@{status='neu'};after=[ordered]@{status=$geprueft};decision='reviewed';references=@('PROP-SYN-001')},
  [ordered]@{id='AUD-ACCEPT-001';event_type='proposal_acceptance';actor_role='customer_reviewer';occurred_at='2026-07-13T09:30:00Z';source_revision='REV-001';before=[ordered]@{status=$geprueft};after=[ordered]@{status='angenommen'};decision='accepted';references=@('PROP-SYN-001','SUP-SYN-001')},
