...
`automation/New-ReleaseCandidate.ps1` verlangt einen expliziten, bereits existierenden `-CandidateSourceCommit` (`-SourceCommit` bleibt als Aufrufalias kompatibel). Der Commit muss `HEAD` sein und darf die eigene Candidate-Evidence noch nicht enthalten. Der Schema-v3-Candidate bindet ihn als `candidate_source_commit/tree`; `source_commit/tree` bleiben null. Er bleibt `CONTRACT_REFERENCE_ONLY` und ist mit `installable_blueprint: false` nicht installierbar.
