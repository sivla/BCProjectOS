...
Die macOS-Evidence MUST in ihren synthetischen Temp-Basispfad wechseln, ihn mit dem POSIX-verfügbaren `/bin/pwd -P` physisch auflösen, das Ergebnis fail-closed als nichtleeren vorhandenen Ordner prüfen und den aufgelösten Pfad als `TMPDIR` an Kindprozesse weiterreichen. Sie darf weder den PowerShell-Alias `realpath`, ein auf macOS nicht vorhandenes GNU-Werkzeug noch eine in nativen PowerShell-Pipelines unzuverlässige `$LASTEXITCODE`-Annahme voraussetzen. Die Produkt-Symlinkprüfung MUST unverändert bleiben und darf nicht pauschal für systemische Alias-Vorfahren geöffnet werden.
