...
Gegenüber `1.1.0-alpha.1` ist der Block additiv und rückwärtskompatibel. Die unveränderlichen Releases bis `spectra-v1.2.0-alpha.5` dokumentieren die schrittweise Plattformabnahme; `alpha.5` schloss Installer und Windows-Evidence, zeigte danach aber eine lokale Remoteidentität in der macOS-Fresh-Clone-Fixture. Die kleinste rückwärtskompatible Korrekturversion ist deshalb ausschließlich `1.2.0-alpha.6`. Bestehende Tags und ihre rote Portabilitätsevidence bleiben unverändert; dieser Change erzeugt noch keinen Candidate oder Release.
