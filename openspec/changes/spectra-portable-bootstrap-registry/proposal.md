...
Gegenüber `1.1.0-alpha.1` ist der Block additiv und rückwärtskompatibel. Die unveränderlichen Releases bis `spectra-v1.2.0-alpha.8` dokumentieren die schrittweise Plattformabnahme; `alpha.8` schloss installierte Untergates und zeigte danach macOS' systemischen `/var`-Alias im synthetischen Temppfad. Die kleinste rückwärtskompatible Korrekturversion ist deshalb ausschließlich `1.2.0-alpha.9`. Bestehende Tags und ihre rote Portabilitätsevidence bleiben unverändert; dieser Change erzeugt noch keinen Candidate oder Release.
