...
Gegenüber `1.1.0-alpha.1` ist der Block additiv und rückwärtskompatibel. Die unveränderlichen Releases bis `spectra-v1.2.0-alpha.9` dokumentieren die schrittweise Plattformabnahme; `alpha.9` schloss den systemischen `/var`-Alias fachlich, zeigte aber danach die Kollision des PowerShell-Alias `realpath` mit der erwarteten nativen Exitcode-Semantik. Die kleinste rückwärtskompatible Korrekturversion ist deshalb ausschließlich `1.2.0-alpha.10`. Bestehende Tags und ihre rote Portabilitätsevidence bleiben unverändert; dieser Change erzeugt noch keinen Candidate oder Release.
