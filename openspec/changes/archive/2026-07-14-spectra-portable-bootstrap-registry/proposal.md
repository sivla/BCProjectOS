...
Gegenüber `1.1.0-alpha.1` ist der Block additiv und rückwärtskompatibel. Die unveränderlichen Releases bis `spectra-v1.2.0-alpha.11` dokumentieren die schrittweise Plattformabnahme; `alpha.11` verwendete das POSIX-verfügbare `/bin/pwd -P`, zeigte aber danach, dass PowerShell in dieser Pipeline keinen verlässlichen `$LASTEXITCODE` bereitstellt. Die kleinste portable Korrektur validiert stattdessen das physische Ergebnis fail-closed als vorhandenen Ordner und ist ausschließlich `1.2.0-alpha.12`. Bestehende Tags und ihre rote Portabilitätsevidence bleiben unverändert; dieser Change erzeugt noch keinen Candidate oder Release.
