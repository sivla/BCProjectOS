...
The repository is the BCProjectOS development workspace for Spectra. A generated workspace will eventually be a separate customer instance and must not be populated with Universaarl-specific or other customer-specific knowledge. This change specifies that future behavior but performs no installation.
...
- Implementing an installable customer-workspace generator, release publication, migration, or product functionality beyond non-installable synthetic-fixture and consumer-binding safeguards.
...
The exact script does not exist in this change. Release identity SHALL be discovered from verified Spectra release Evidence, not asserted by the operator. `-ExpectedBlueprintVersion`, if supplied, is only a comparison value and MUST match the verified release version exactly. It is never sufficient release Evidence. A later test-only mode MAY use an explicit `-SyntheticFixture` switch, but it MUST NOT publish an installable customer workspace.
...
- a correctly named annotated release tag `spectra-v<SemVer>` exists;
...
Missing, unknown, incomplete, or contradictory release state MUST abort with non-zero exit and retain `PENDING_BCPROJECTOS_RELEASE`. The manifest intentionally contains no `release_commit`; the externally resolved commit of the annotated tag is the release commit.
...
`workspace.schema.json` currently requires SemVer values for `blueprint_version` and `created_with_version`. This change does not invent a placeholder SemVer and does not modify the schema or product contract. The existing versioned minimal-contract and POV files are historical synthetic evidence, not prospective fixtures and never generator or consumer input. A pre-release synthetic fixture that lacks verified version Evidence MAY be intentionally schema-incomplete for negative testing, but MUST NOT pass customer-workspace validation. A positive schema-valid installable workspace test requires verified installable release Evidence.

### Decision: Consumer Binding Is Pending Until Externally Verified

The product provides `schemas/consumer-binding.schema.json` and a read-only validator. `PENDING_BCPROJECTOS_RELEASE` contains only the product ID and canonical repository URL; every release field is `null`. `BOUND` requires an externally verified annotated tag, its resolved tag commit, final manifest at that commit, manifest source ancestry, consumer-mode/installability consistency, and matching SHA-256 payload digest. A structurally plausible BOUND fixture without those checks is rejected. No binding is emitted by this repository while release evidence is absent.
...
- complete and consistent tag, externally resolved tag commit, final manifest, source ancestry, installable-release claim, product-scope digest, and optional expected-version match;
- consumer binding status, including null-only pending fields and externally verified bound fields;
- explicit rejection of synthetic fixtures as installable customer workspaces.
...
- **Release Evidence contract needs an installable manifest shape:** No final installable manifest or annotated release tag exists. MVP-1 implementation must resolve those external facts without adding a circular `release_commit` field.
